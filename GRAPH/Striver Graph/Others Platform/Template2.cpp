...
ll findIndex(ll i, ll n, ll pref[], ll x) {
...
void solve() 
{
	...
	for(ll i = 0; i < n; i++) {
		if(arr[i] > x) {
			if(i != n - 1) dep[i + 1].push_back(i);
			continue;
		}

		ll ind = findIndex(i, n, pref, x);
		if(ind < n) dep[ind].push_back(i);
		value[i] = ind - i - 1;
	...
	for(ll i = n - 1; i >= 0; i--) {
		for(auto j : dep[i]) value[j] += value[i];
	}

	ll ans = sumOfArr(n, value);
